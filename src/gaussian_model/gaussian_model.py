...
from gaussian_utils import inverse_sigmoid, BasicPointCloud, RGB2SH
...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int, args):
        # Initialize the state of Gaussian
        ...
        self.spatial_lr_scale = 0  # --> learning rate
        self._deformation_table = torch.empty(0)  # --> search table for deformation information
    ...
    def capture(self):
        # capture state and params of current implementation
        return (
            self.active_sh_degree,
            self._xyz,
            #self._deformation.state_dict(),  # --> .state_dict() : Dictionary containing all parameters of the model
    ...
    def restore(self, model_args, training_args):
        # Previously saved model state and configuration is loaded back into the model instance
        (self.active_sh_degree,
         self._xyz,
         deform_state,
         self._deformation_table,
        ...
        self.training_setup(
            training_args)  # training_args: -->  configuration of training : lr , param of optimizer etc.
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        self.optimizer.load_state_dict(opt_dict)

    @property  # @property: --> readonly property
    ...
    def oneupSHdegree(self):  # --> SH degree + 1
    ...
    def training_setup(self, training_args):
        # update learning params in training
        ...
        l = [
            ...
            ####### get_grid_parameters： --> computation about grid
        ...
        # --> get_expon_lr_func : adjust learning rate with training process
    ...
    def update_learning_rate(self, iteration):
        ''' Learning rate scheduling per step
            --> different parameters apply different update method
        '''
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")   # --> to GPU
        self._deformation_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)  ## --> to GPU


    # LOAD FROM PLY
    def load_ply(self, path):
        ''' load model params from ply file '''
    ...
    #     '''  Load pre-trained model weights and associated data from a specified path into a model instance '''
    #
    #     print("loading model from exists{}".format(path))
    #     weight_dict = torch.load(os.path.join(path,"deformation.pth"),map_location="cuda")
    #     self._deformation.load_state_dict(weight_dict) # --> load_state_dict : apply weight to model
    #     self._deformation = self._deformation.to("cuda")
    #     self._deformation_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)
    #     self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")  # --> _deformation_accum: accumulate deformation information
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ''' repace parameter group tensor to optimizer '''
    ...
    def _prune_optimizer(self, mask):
        ''' Improved computational efficiency of models '''
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ''' Adding and adjusting the number of Gaussian points '''
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size):
        ''' remove points with bad opacity, scale, or out of screen '''
    ...
    #     ''' Calculates and accrues the smoothness
    #         at a particular level in a model consisting of a multi-resolution grid.
    #     '''
    #
    #     # --> deformation model can calculate grid information
    #     multi_res_grids = self._deformation.deformation_net.grid.grids
    #     total = 0
    #     # model.grids is 6 x [1, rank * F_dim, reso, reso]
    #
    #     # --> Determine if the time dimension is included
    ...
    #     ''' error of position, rotation, scaling '''
    #
    ...
    #     ''' Applying a random perturbation to a selected set of points
    #         and ensure that the perturbed points still lie inside a well-defined axis-aligned bounding box (AABB)
    #         data argumentation
    #     '''
    ...
    #     new_xyz, mask = self.get_displayment(selected_xyz, self.get_xyz.detach(),perturb) # -> mask from perturbed points
    #
    #     # -> extract new feature by mask
    ...
    #
    #     # -> add points which are still stable after perturbation
    #     self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table)
    #     return selected_xyz, new_xyz

    # def compute_deformation(self,time):
    #     ''' compute deformation and translation '''
    #     deform = self._deformation[:,:,:time].sum(dim=-1)  # --> _deformation : deformation data from network    time 0 -> t
